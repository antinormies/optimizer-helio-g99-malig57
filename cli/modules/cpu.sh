...
if [ -n "$BBS" ]; then
  $ADB shell settings put global persist.added_boot_bgservices "$BBS"
...
if [ -n "$ZPT" ]; then
  $ADB shell settings put global persist.zygote.preload_threads "$ZPT"
  echo "  zygote_preload_threads = $ZPT"
fi

# --- Power mode: interactive ---
$ADB shell setprop persist.sys.powerhal.interactive 1
